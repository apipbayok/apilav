...
export default function FormKordes() {
  ...
  const handleSelectSuggestion = (suggestion) => {
    ...
    getParentKec(suggestion.kecamatan_id);
    ...
    if (user.id) {
      ...
      axiosClient.post("/daftar", user)
        .then(() => {
          ...
          setPesan(err.response.data.message)
        setShowGagal(true)
        // console.log()
      })
    }
  ...
  return (
    <div className="container-fluid">
      ...
      <h2>Form Data Kordinator Desa</h2>
            ...
            <Card>
              <CardBody>
                ...
                <Form.Select aria-label="Default select" value={user.kecamatan_id}
                             onChange={(e) => kecChangeHandle(
                               e.target.value, e.target[e.target.selectedIndex].getAttribute('data-dapil')
                             )}>
                ...
                <Form.Group style={{marginTop: '1.5rem'}}>
                  <Form.Label>Up Link Kordinator Kecamatan</Form.Label>
                  <Form.Select aria-label="Default select" value={user.parent}
                    ...
                    <option key="koid" value="0">Silahkan Pilih Kordinator Kecamatan</option>
                    {lsKorcam.map(u => (<option key={u.id} data-segmen={u.segmen_id} data-subsegmen={u.subsegmen_id} value={u.id}>{u.nama}</option>))}
                  </Form.Select>
                </Form.Group>
              </CardBody>
              <CardFooter>
