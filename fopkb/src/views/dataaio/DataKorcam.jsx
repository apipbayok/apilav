...
export default function DataKorcam() {
  ...
  const kolom = [
    ...
    { name: "Seg/SubSeg", selector: row => row.seg_nama},
    {
      name: "HP", cell: row => (
        <div>
          {row.hp}
    ...
    //         <Link className="nav-link" to={'/users/' + { linkurl } + '/' + row.id}>
  ...
  const getUsers = () => {
    axiosClient.get(`/userko/KORCAM`).then(({data}) => {
      // console.log(data)
      setUsers(data)
    })
  }
  ...
  const onSubmit = ev => {
    ev.preventDefault()
    axiosClient.post(`/userko/KORCAM`, payload)
      .then(({ data }) => {
        setUsers(data);
      }).catch(err => {
      console.log(err)
  ...
  useEffect(() => {
    ...
    getKec();
    getDesa();
    // console.log(datambijage)
  }, [])

  return (
    <div className="container-fluid">
      <div className="col">
        <h3>Data Kordinator Kecamatan</h3>
      </div>
      <Form onSubmit={onSubmit} style={{marginBottom:'2rem'}}>
        <Row>
          ...
          <div className="col">
            ...
            <button className="btn btn-success"><Link state={datambijage} to={"/korcam/add"} className="nav-link">
